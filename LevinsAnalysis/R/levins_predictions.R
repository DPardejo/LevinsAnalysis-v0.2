...
#'Community Network matrix \code{J} variables due to effect of an input in a
#'variable of the same matrix.
#'
...
#'
#'
#'@details \itemize{ \item Through the argument \code{RM} the user can define
#'  the type of random distribution method to be used on the random assignment
#'  of values of interaction strength for the variable pairs of the Community
#'  Matrix \code{J}, by choosing the appropriate option as follows: \itemize{
#'  \item \code{"uniform"} random assignment method follows a Uniform
#'  probability distribution (default); \item \code{"normal"} random assignment
#'  method follows a Normal probability distribution; \item \code{"pareto"}
#'  random assignment method follows a Pareto probability distribution; \item
#'  see \code{Note} for more information and \code{Examples} for practical use.}
#'  \item Through the use of matrices for the arguments \code{INT_MIN} and
#'  \code{INT_MAX} the user can change the default interval values for the
#'  random assignment ([1e-6, 1]). This can be done by use of square matrices of
#'  the same length and writing format as the Community Network matrix
#'  \code{J}, as follows: \itemize{ \item \code{INT_MIN} the user defined
#'  values for each variable pair will be used by the process as the minimum
#'  value in the interval for random assignment ([x; 1]); \item \code{INT_MAX}
#'  the user defined values for each variable pair will be used by the process
#'  as the maximum value in the interval for random assignment ([1e-6; x]);
#'  \item the user defined values in \code{INT_MIN} and \code{INT_MAX} should be
#'  within the interval (0, 1]; \item to define an exact interaction strength
#'  value for any given variable pair (and not an interval), the user should
#'  indicate same value in both matrices; \item for variable pairs which the
#'  user does not pretend to change the default interval values, fill with value
#'  \code{0}; \item see \code{Note} for more information and \code{Examples} for
#'  practical use.} \item To compute a \code{interaction Strength Matrix} the
#'  program will compute \code{x} number of random matrices which will then be
#'  screened for their stability, with those accepeted being used to compute an
#'  average interaction strength matrix. Through the use of the argument
#'  \code{NT} the user can define the number of runs of the process. The user
#'  defined number of runs follows the equation: \code{Number of Runs = (Number of
#'  variables of J * NT)}. See \code{Examples} for pratical use; \item
#'  \code{fname} should be used giving the name intended for the exporting files
#'  noting that a suffix \code{"_levins_pred.txt"} will be added to the file
#'  name. See \code{Examples} for pratical use.}
#'
...
#'  stable matrices used for the calculations, matrices with the percentage of
#'  runs which gave positive, negative or null predicitions for each variable
#'  pair and Levins Table of Predictions.
#'
...
#'@concept interaction, levins, loop, prediction, network, sign, strength
#'
#'@seealso \code{\link{cm_strength}}, \code{\link{cm_stability}},
#'  \code{\link{levins_concurrent}}, \code{\link{J}}, \code{ \link{pred_correlation}},
#'  \code{\link{pred_graph}}
#'
#' @examples ## To compute the table of predictions, without printing the results to a .txt file
#' levins_prediction(MM, RM = "uniform", INT_MIN = NA, INT_MAX = NA, NT = 1000, fname = NA)
#' ## or (to assign the invisible results list)
#' results <- levins_prediction(MM, RM = "uniform", INT_MIN = NA, INT_MAX = NA, NT = 1000, fname = NA)
#'
#' ## To compute the table of predictions, printing the results to a .txt file
#' #' levins_prediction(MM, RM = "uniform", INT_MIN = NA, INT_MAX = NA, NT = 1000, fname = "MM")
#'
#' ## To compute the table of predictions, using a Normal distribution
#' levins_prediction(MM, RM = "normal", INT_MIN = NA, INT_MAX = NA, NT = 1000, fname = NA)
#'
#' ## To compute the table of predictions, using a Pareto distribution
#' levins_prediction(MM, RM = "pareto", INT_MIN = NA, INT_MAX = NA, NT = 1000, fname = NA)
#'
#' ## To compute the table of predictions, using a user defined minimum strength interval value
#' matrix_min<-matrix(rep(0,(length(MM))), nrow = sqrt(length(MM)))
#' matrix_min[1, 5]<- 0.3
#' levins_prediction(MM, RM = "pareto", INT_MIN = matrix_min, INT_MAX = NA, NT = 1000, fname = NA)
#'
#' ## To compute the table of predictions, using a user defined maximum strength interval value
#' matrix_max<-matrix(rep(0,(length(MM))), nrow = sqrt(length(MM)))
#' matrix_max[1, 5]<- 0.7
#' levins_prediction(MM, RM = "pareto", INT_MIN = NA, INT_MAX = matrix_max, NT = 1000, fname = NA)
#'
#' ## To compute the table of predictions, where [1, 5] pair has user defined fixed strength value
#' matrix_min<-matrix(rep(0,(length(MM))), nrow = sqrt(length(MM)))
#' matrix_min[1, 5]<- 0.4
#' matrix_max<-matrix(rep(0,(length(MM))), nrow = sqrt(length(MM)))
#' matrix_max[1, 5]<- 0.4
#' levins_prediction(MM, RM = "pareto", INT_MIN = matrix_min, INT_MAX = matrix_max, NT = 1000, fname = NA)
#'
#' ## To compute the table of predictions, with user defined number of runs
#' levins_prediction(MM, RM = "uniform", INT_MIN = NA, INT_MAX = NA, NT = 3000, fname = NA)
#'
#'
...
#'
#'
levins_predictions <- function(J, RM = "uniform", INT_MIN = NA, INT_MAX = NA, NT = 1000, fname = NA){
	##
	...
	if(is.matrix(J) == FALSE | is.numeric(J) == FALSE){
		...
		if(n_rows == n_cols){
			...
			if(n_stable > 0){
				...
				cat("\n\n")
				cat(paste("the probability distribution used is:", list_results[[3]], sep = " "))
				cat("\n\n\n")
			...
			if(is.character(fname) == TRUE){
				file_name_txt <- paste(fname, "_levins_predictions.txt", sep = "")
