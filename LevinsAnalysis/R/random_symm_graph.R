random_symm_graph <- function(J, fname = NA){
	##
	## error message if the input is not a square numeric matrix
	if(is.matrix(J) == FALSE | is.numeric(J) == FALSE){
		cat("\nerror: the input must be a square, numeric and symmetric matrix")
		cat("\n\n")
	}
	else{
		...
		## square matrix check
		{
		if(n_rows == n_cols & isSymmetric(JS) == TRUE){
			{
			if(length(fname) > 1){
			...
			repeat{
				...
				g_gnm <- erdos.renyi.game(n_rows, nondia_tot/2, type = c("gnm"),
				...
				if(is.connected(g_gnm) == TRUE & rotazioni <= 1000){
					...
					##
					## preparing the symmetric links in the community matrix
					{
					...
					nondia_elements_1 <- nondia_elements[1:(l_nondia_tot/2)]
					nondia_elements_2 <- nondia_elements[(1 + l_nondia_tot/2):l_nondia_tot]
					##
					for(i in 1:nrow(edgelist_g)){
						M_g[edgelist_g[i,1],edgelist_g[i,2]] <- nondia_elements_1[i]
						M_g[edgelist_g[i,2],edgelist_g[i,1]] <- nondia_elements_2[i]
					}
					##
					## preparing the diagonal of the symmetric community matrix
					...
					if(is.na(fname) == FALSE){
						file_name_txt <- paste(fname, "_random_symm_graph.txt", sep = "")
					...
					if(rotazioni > 1000){
						...
						cat("please, verify the numeric matrix used as input is connected\n\n")
						invisible(NULL)
						break
						}
					}
